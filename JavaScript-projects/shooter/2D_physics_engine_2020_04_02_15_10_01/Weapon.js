class Weapon {
  ...
  fire() {
    ...
    if (this.clock > this.bulletPeriod) {
      let posBullet = p5.Vector.add(this.owner.pos, this.owner.pointer);
      let bullet = new Bullet(posBullet, this.owner.pointer.copy(), this.bulletSpeed, this.bulletAceleration) ;
...
function getWeapon(owner, type) {
  switch (type) {
    case 'default':
      return new Weapon(owner, 10, 1, 10, 20, 1, 'straight');
    default:
