...
//Renvoit un vecteur de toutes les lignes definies par la balise:
	  ...
	  for(std::vector<std::string>::const_iterator run=ID.begin();run!=ID.end();++run)
	    {
		  ...
		  for(std::vector<std::string>::const_iterator runOPT=OPTIONS.begin();runOPT!=OPTIONS.end();++runOPT)
			...
			//Pour chaque couple on parcourt le fichier:
			...
			while(std::getline(read,line_code))
				   ...
				   if(POS_START!=std::string::npos)
					{
					   ...
					   if( (POS_ID!=std::string::npos) | (*run=="*") )
						...
						// ID OK ET UNE OPTION OK,ou pas d'option on prend
							...
							//Verifie qu'il y ait balise # pour les descripteurs:
			 				 if(POS_DES!=std::string::npos)
			    				{
								//On prend la bonne colonne:
			   ...
			   if(wordslist.size()==0)
				{
				   cout<<"@getData -> Impossible de trouver ID ou OPTIONS suivants dans le path:"<<path<<endl;
				   cout<<"ID: "<<*run<<endl;
				   cout<<"OPTION: "<<*runOPT<<endl;
				   exit(0);
				} 
...
// A surcharger en appel, pour faire des appels avec des vecteurs de Id et options!!!Pas possible pour le moment
				...
				for(std::vector<std::string>::const_iterator run=parameters.begin();run!=parameters.end();++run)
				 {
					...
					isParam=isParameters(line); //continue seulement si dernier mot appelé appelle à son tour mais pes les intermediaires non? A CHECKER,PAS EXACTEMENT RECURSIF SAUF SI UN SEUL BLOC PARAMETRE SUR CHAQUE MOT APPELANT
...
//appelle get_data et renvoie la nouvelle liste de lignes définies par les parametres
...
int DATA::get_colonne(std::string line,std::string requestDES)
{
  ...
  //EXTRACTION DESCRIPEUR dans l'ordre, donc l'ordre des elements dans le vecteur donne l'ordre des colonnes et permet de recuperer la bonne colonne désirée:
   do
	...
	//Renvoie une colonne au hasard parmi les colonnes existantes:
	if(requestDES=="!") 
	 {
		return m_maths.RANDOM() % descripteurs.size()+1;
	 }
  
  
  //SINON ON PREND CELLE DEMANDEE:
...
{	
	...
	//COMPTER LE NOMBRE DE COLONNES:(=nbre point-virgules +1)
...
//Les blocs parametres peuvent être placées n'importe où dans l'appelant! MAIS PAS D APPELS RECURSIFS!
//Renvoie une phrase,indexée sur la balise d'appel, qui est un string contenant des blocs parametres (a trou) en remplacant les blocs d'appels par un seul et unique mot.
std::string DATA::getTAT(const std::string& path,std::string ID,std::string OPTIONS,std::string DESCRITPEURS)
{
	
		std::vector<std::string> linepack,VEC_ID,VEC_OPTIONS;
		std::string line;
		
		VEC_ID.push_back(ID);
		VEC_OPTIONS.push_back(OPTIONS);
		//1er appel,liste faite des mots appelés:
		linepack=getData(path,VEC_ID,VEC_OPTIONS,DESCRITPEURS);
		//Prends une ligne au hasard:
		line=pickInlist(linepack);
		//renvoie la phrase completée:
		return completeTAT(line);
	
}


//Renvoie un string ou les blocs parametres d'un string ont été remplacées par des mots
std::string DATA::completeTAT(std::string line)
{
	
	std::string regular,parameter,first,trou;
	std::vector<std::string> all,parameters;
	bool texteOnly=false;
	bool isParam=true;
	std::size_t POS_L,POS_R;
	...
	if(checkParameters(line))
	{
		...
		do
		{
			...
			//Prends le texte avant le bloc parametre, pe une chaine vide ou tout le reste du string:
			if(POS_L!=std::string::npos)
			{
				regular = line.substr(0,POS_L);
				isParam=true;	
			}
			...
			//ajoute le texte:
			if(regular.size()!=0) all.push_back(regular);
			
			
			if(isParam)
			{
				//Isole le parametre
				parameter = line.substr(POS_L+1,POS_R-(POS_L+1));
				//cout<<"parameter:"<<parameter<<endl;
				//lit le bloc parametre et renvoie un vecteur contenant toutes les lignes y correspondant:
				parameters=readParameters(parameter);
				//Selectionne une ligne parmi toutes:
				trou=pickInlist(parameters);
				//Prends le mot dans la bonne colonne:
				trou=takeFirst(trou);
				//Ajoute a all
				all.push_back(trou);
				
				//Reprend la lecture apres le bloc parametre:
				line=line.substr(POS_R+1);
				//cout<<"ligne restante:"<<line<<endl;
				isParam=false;
		      }
	...
	{
		cout<<"@Erreur completeTAT impossible, [] manquants sur la ligne:"<<line<<endl;
		exit(0);
	}
